...
class NoisyNERDataset(BaseDataset):
    ...
    def fit_vocabulary(samples: Iterator[TokensSample]) -> Vocabulary:
        ...
        :param samples: a collection of samples from which we can compute the possible classes
        :return: the Vocabulary containing all the possible classes in your dataset
        """
    ...
    def dataset_iterator_func(self) -> Iterable[Dict[str, Any]]:
        ...
        :return: an iterable over all the transformed elements in the dataset
        """

        samples_iterable: Iterable[TokensSample] = self.samples_iterator()

        for token_sample in samples_iterable:

            noisify_tokens = np.random.uniform(0, 1) < self.noisification_probability

            if noisify_tokens:
                input_ids, token_offsets = self._tokenize([token.lower() for token in token_sample.tokens])
            else:
    ...
    def _tokenize(self, tokens: List[str]) -> Optional[Tuple[torch.Tensor, List[Tuple[int, int]]]]:
